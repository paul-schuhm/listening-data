...
if (
    ...
    !isset($config['redirect_uri'])  ||
    !isset($config['authorize_url'])  ||
    !isset($config['base_url'])  ||
    !isset($config['access_token_url'])
) {
    throw new Exception("Fournir les informations concernant les services de Spotify (url), client_id et client_secret nécessaires à la bonne exécution des programmes.");
}
...
//define('REDIRECT_URI_SECURE', $config['redirect_uri_secure']); Not necessary
